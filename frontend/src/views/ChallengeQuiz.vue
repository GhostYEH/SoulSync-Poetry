<template>
  <div class="quiz-page">
    ...
    <main v-if="level && !levelFinished" class="quiz-layout">
      ...
      <section class="quiz-content">
        ...
        <div class="quiz-actions">
          ...
          <button v-else type="button" class="primary-action" @click="goNext">{{ questionIndex === questions.length - 1 ? '完成本关' : '下一题' }} <PhArrowRight :size="18" /></button>
...
function submitAnswer() {
  ...
  result.value = normalize(answer.value) === normalize(correctAnswer.value)
  submitted.value = true
  answeredQuestions.value[questionIndex.value] = true
  api.challenge.submitAnswer({
...
function finishLevel() {
  ...
  api.challenge.updateProgress(levelNumber.value).catch(() => {})
